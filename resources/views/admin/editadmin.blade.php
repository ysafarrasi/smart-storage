        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{ __('users.EditAdmin') }}</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form action="{{ route('daftaradmin.update', $user->id ?? '') }}" method="POST" class="row g-3">
                                @csrf
                                @method('PUT')
                                <div class="col-md-12">
                                    <label for="name" class="form-label">{{__('users.Namaadmin') }}</label>
                                    <input type="hidden" name="id" value="{{ $user->id ?? '' }}">
                                    <input type="text" class="form-control" name="name" value="{{ old('name', $user->name) }}" placeholder="Masukkan nama" required>
                                </div>
                                <div class="col-md-12">
                                    <label for="email" class="form-label">Email</label>
                                    <input type="email" class="form-control" name="email" value="{{ old('email', $user->email) }}" placeholder="Masukkan email" required>
                                </div>
                                <div class="col-md-12">
                                    <label for="password" class="form-label">{{ __('users.Masukan_Password') }}</label>
                                    <input type="password" class="form-control" name="password" placeholder="{{ __('users.Masukan_Kata_Sandi_Admin') }}" autocomplete="new-password">
                                    <small class="text-muted">{{ __('users.minimal8') }}</small>
                                </div>
                                <div class="col-md-12">
                                    <label for="password_confirmation" class="form-label">{{ __('users.Ulangi_Kata_Sandi') }}</label>
                                    <input type="password" class="form-control" name="password_confirmation" placeholder="{{ __('users.Ulangi_kata_Sandi_Admin') }}" autocomplete="new-password">
                                </div>
                                <div class="col-12">
                                    <button type="submit" class="btn btn-warning">{{ __('users.simpanperubahan') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
